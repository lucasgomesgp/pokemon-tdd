...
export function Card({ id, title, type, url }: CardProps) {
  const [toggleClickFlex, setToggleClickFlex] = useState(false);


  return (
    <button
      className={clsx(
        "flex min-w-[300px] bg-background-home shadow-lg relative px-2 py-5 items-center justify-center rounded-2xl border-2 hover:opacity-80 cursor-pointer transition-all animate-opacity ",
        {
          "flex-col": toggleClickFlex === false,
          "flex-row px-4": toggleClickFlex,
        }
      )}
      key={url}
      onClick={() => {
        setToggleClickFlex(!toggleClickFlex);
      }}
    >
      <p className={getColorOfIdPokemon(type, toggleClickFlex)}>#{id}</p>
      <div
        className={clsx("flex bg-white rounded-full p-2 mb-2 relative", {
          "mt-8": toggleClickFlex,
        })}
      >
        <img
          src={url}
        ...
        >
          <g filter="url(#filter0_i_102_754)">
            <path
              ...
              className={getColorOfPokemonCircle(type)}
            />
          </g>
          <defs>
            <filter
